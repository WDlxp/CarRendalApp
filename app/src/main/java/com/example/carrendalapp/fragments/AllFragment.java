 ...
 * @author WD
 */
public class AllFragment extends BaseLazyLoadFragment {
    ...
    public void showCar() {//将后台数据库表中的相关信息反馈到前端展示
        //适配器
        appointOrderAdapter = new AppointOrderAdapter(getContext(), carList, 0);
        //将信息放入lv_list显示出来
        ...
        new QueryAppointOrderTask().execute(account);
    }

    /**
     * 查询全部信息
     */
    private class QueryAppointOrderTask extends AsyncTask<String, Void, List<AppointOrder>> {
        ...
        protected List<AppointOrder> doInBackground(String... strings) {
            ...
            try {
                URL url = new URL(UrlAddress.QUERY_ORDER_URL + "?operation=queryAll&account=" + account);
